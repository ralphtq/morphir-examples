...
rulesTest =
    ...
    describe "Rules tests"
        [ test "foo" <| \_ -> Expect.equal "False" (Debug.toString False)
        , test "isApplicable negative vs -1" <| \_ -> Expect.equal True (isApplicable -1 negative) -- "expected to apply for negative"
        , test "isApplicable negative vs 0" <| \_ -> Expect.equal False (isApplicable 0 negative) -- "expected not to apply for zero"
        , test "isApplicable negative vs 1" <| \_ -> Expect.equal False  (isApplicable 1 negative) --  "expected not to apply for positive"
        , test "isApplicable positive vs -1" <| \_ -> Expect.equal True  (isApplicable -1 negative)  --  "expected not to apply for negative"
        , test "isApplicable positive vs 0" <| \_ -> Expect.equal False  (isApplicable 0 negative) -- "expected to apply for zero"
        , test "isApplicable positive vs 1" <| \_ -> Expect.equal False   (isApplicable 1 negative) --  "expected not to apply for positive"
        , test "isApplicable zero vs -1" <| \_ -> Expect.equal True   (isApplicable -1 negative) -- "expected to not apply for negative"
        , test "isApplicable zero vs 0" <| \_ -> Expect.equal False   (isApplicable 0 negative) -- "expected not to apply for zero"
        , test "isApplicable zero vs 1" <| \_ -> Expect.equal False  (isApplicable 1 negative) --  "expected to apply for positive"
